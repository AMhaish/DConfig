(function (namespace, $) {
    ...
    p.initialize = function () {
        ...
        this._initRadioOtherControls();
    };
    p.initializePanelsHeights = function () {
        this._initAppsMenuDetailsLayout();
    }
    // =========================================================================
    // PANELS LAYOUT
    // =========================================================================

    p._initAppsMenuDetailsLayout = function () {
        var Menu_SubMenu_Details = $('#Menu_SubMenu_Details');
        Menu_SubMenu_Details.height($(window).height() - 93);
        var subMenuContainer = $('.subMenuContainer');
        var detailsContainer = $('.detailsContainer');
        var searchListTopMenu = $('.searchListTopMenu');
        var searchListContent = $('#searchListContent');
        var filterListTopMenu = $('.filterListTopMenu');
        if (searchListTopMenu.length > 0) {
            searchListContent.height(Menu_SubMenu_Details.height() - searchListTopMenu.outerHeight());
        }
        if (filterListTopMenu.length > 0) {
            searchListContent.height(Menu_SubMenu_Details.height() - filterListTopMenu.outerHeight());
        }
        var closeOpenButton = $('#subMenuOpenCloseBtn');
        var menuOpened = true;
        if (closeOpenButton.length > 0) {
            closeOpenButton.click(function () {
                if (menuOpened) {
                    menuOpened = false;
                    closeOpenButton.removeClass('fa-caret-left');
                    closeOpenButton.addClass('fa-caret-right');
                    subMenuContainer.css('width', 0);
                    subMenuContainer.css('border-style', 'none');
                    detailsContainer.css('width', '100%');
                } else {
                    menuOpened = true;
                    closeOpenButton.removeClass('fa-caret-right');
                    closeOpenButton.addClass('fa-caret-left');
                    subMenuContainer.css('width', '');
                    subMenuContainer.css('border-style', '');
    ...
    // OTHER CONTROL
    // =========================================================================
    p._initRadioOtherControls = function () {
        $('.other input[type="radio"]').change(function () {
            var textBox = $(this).parents('.radioOtherControl').find('.otherBox');
            var attr = $(this).attr('data-enable-other');
            switch (attr) {
                case "true":
