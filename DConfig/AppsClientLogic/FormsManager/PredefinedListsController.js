angular.module('DConfig').controllerProvider.register('FormsManager.PredefinedListsController', function ($scope, $location, BreadCrumpsProvider, UserMessagesProvider, FormsManagerDataProvider, EventsProvider, $modal, scopeService) {
    ...
    function initialize() {
        BreadCrumpsProvider.breadCrumps.path = ['Forms Manager', 'Predefined Lists'];
        UserMessagesProvider.displayProgress(1);
        FormsManagerDataProvider.getFormsFieldsEnumsTree().then(function (data) {
                        ...
                        'check_callback': function (operation, node, node_parent, node_position, more) {
                            ...
                            if (operation === 'delete_node' && $scope.CurrentExecutingContext == $scope.ExecutingContexts.None) {
                                UserMessagesProvider.confirmHandler("Are you sure you want to delete this node?", function () {
                                    ...
                                    FormsManagerDataProvider.deleteFormsFieldsEnum($scope.currentObj).then(function (returnValue) {
    ...
    $scope.createNode = function () {
        ...
        modalInstance.result.then(function (newNode) {
            UserMessagesProvider.displayLoading();
            FormsManagerDataProvider.createFormsFieldsEnum(newNode).then(function (returnValue) {
                UserMessagesProvider.hideLoading();
                if (returnValue.data.result == 'true') {
    ...
    $scope.saveChanges = function () {
        if ($scope.detialsForm.$valid) {
            if ($scope.currentObj != null && ($scope.CurrentExecutingContext == $scope.ExecutingContexts.None || $scope.CurrentExecutingContext == $scope.ExecutingContexts.Renaming)) {
                ...
                FormsManagerDataProvider.updateFormsFieldsEnum($scope.currentObj).then(function (returnValue) {
                    UserMessagesProvider.hideLoading();
                    if (returnValue.data.result == 'true') {
    ...
    $scope.saveFields = function () {
        UserMessagesProvider.displayLoading();
        var distNode = listsTreeContainer.jstree(true).get_node($scope.currentNodeObject);
        FormsManagerDataProvider.updateFormsFieldsEnumValues($scope.currentObj.Id, { Values: $scope.currentObj.Values }).then(function (returnValue) {
            UserMessagesProvider.hideLoading();
            if (returnValue.data.result == 'true') {
    ...
    $scope.removeField = function (index) {
        ...
        UserMessagesProvider.confirmHandler("Are you sure you want to delete this item?", function () {
            var distNode = listsTreeContainer.jstree(true).get_node($scope.currentNodeObject);
            UserMessagesProvider.displayLoading();
            FormsManagerDataProvider.deleteFormsFieldsEnumValue($scope.currentObj.Values[index].Id).then(function (returnValue) {
                UserMessagesProvider.hideLoading();
                if (returnValue.data.result == 'true') {
                    $scope.currentObj.Values.splice(index, 1);
