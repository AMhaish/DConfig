angular.module('DConfig').controllerProvider.register('Gallery.GalleryController', function ($scope, $location, BreadCrumpsProvider, UserMessagesProvider, GalleryDataProvider, EventsProvider, $modal, IntentsProvider, scopeService, $upload) {
    ...
    var foldersTreeContainer = $('#foldersTree');
    var foldersSearchBox = $('#foldersSearchBox');
    $scope.Selected = { SelectedImages: [] };

    function initialize() {
        BreadCrumpsProvider.breadCrumps.path = ['Gallery'];
        UserMessagesProvider.displayLoading();
        GalleryDataProvider.getFoldersTree().then(function (data) {
            $scope.foldersTree = data.data;
    ...
    $scope.createNode = function (e, data) {
        if ($scope.CurrentExecutingContext == $scope.ExecutingContexts.Creating) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'gallery_createForm.html',
    ...
    $scope.uploadDirectory = function () {
        var modalInstance = $modal.open({
            templateUrl: 'uploadDirectoryForm.html',
    ...
    $scope.uploadedDirectory = function () {
        $scope.uploadedDirectoryData = [];
        $scope.uploadedDirectoryData = $scope.newFile.Files;
    }
...
var uploadDirectoryController = function ($scope, $modalInstance, path, UMP, uploadProvider, scopeService) {        
    var uploadedFiles = 0;
    $scope.newFile = { Name: '', Path: path, FilePath: '', Files: [] };
    $scope.progress = 0;
    $scope.createForm = {};
    ...
    $scope.chooseDirectoryClick = function () {
        var picker = document.getElementById('picker');
        var listing = document.getElementById('listing');

        picker.addEventListener('change', e => {
            listing.innerHTML = "";
            $scope.newFile.Files = e.target.files;
            ...
            console.log($scope.newFile.Files);
        });
    }
    ...
    $scope.submit = function () {       
        for (var i = 0; i < $scope.newFile.Files.length; i++) {
            $scope.upload = uploadProvider.upload({
                ...
                method: 'POST',
                data: { Path: $scope.newFile.Path },
                file: $scope.newFile.Files[i], // single file or a list of files. list is only for html5
                //fileName: 'doc.jpg' or ['1.jpg', '2.jpg', ...] // to modify the name of the file(s)
                //fileFormDataName: myFile, // file formData name ('Content-Disposition'), server side request form name
                // could be a list of names for multiple files (html5). Default is 'file'
                //formDataAppender: function(formData, key, val){}  // customize how data is added to the formData. 
                ...
                if (data.data.result == 'true') {
                    ...
                    UMP.errorHandler(999, data.message);
                }
            });
            //.error(...)
            //.then(success, error, progress); // returns a promise that does NOT have progress/abort/xhr functions
