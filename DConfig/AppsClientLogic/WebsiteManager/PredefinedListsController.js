angular.module('DConfig').controllerProvider.register('WebsiteManager.PredefinedListsController', function ($scope, $location, BreadCrumpsProvider, UserMessagesProvider, WebsiteManagerDataProvider, EventsProvider, $modal, scopeService) {
    ...
    function initialize() {
        ...
        WebsiteManagerDataProvider.getViewFieldsEnumsTree().then(function (data) {
            $scope.listsTree = data.data;
            $scope.lists = [];
            for (i = 0; i < data.data.length; i++) {
                $scope.lists.push(data.data[i].obj);
            }
            listsTreeContainer
                        ...
                        'check_callback': function (operation, node, node_parent, node_position, more) {
                            ...
                            if (operation === 'delete_node' && $scope.CurrentExecutingContext == $scope.ExecutingContexts.None) {
                                UserMessagesProvider.confirmHandler("Are you sure you want to delete this node?", function () {
                                    ...
                                    WebsiteManagerDataProvider.deleteViewFieldsEnum($scope.currentObj).then(function (returnValue) {
    ...
    $scope.createNode = function () {
        ...
        modalInstance.result.then(function (newNode) {
            UserMessagesProvider.displayLoading();
            WebsiteManagerDataProvider.createViewFieldsEnum(newNode).then(function (returnValue) {
                UserMessagesProvider.hideLoading();
                if (returnValue.data.result == 'true') {
                    listsTreeContainer.jstree(true).create_node('#', { text: newNode.Name, obj: returnValue.data.obj }, 'last', null, false);
                    $scope.lists.push(returnValue.data.obj);
                    UserMessagesProvider.successHandler();
                }
                else {
    ...
    $scope.saveChanges = function () {
        if ($scope.detialsForm.$valid) {
            if ($scope.currentObj != null && ($scope.CurrentExecutingContext == $scope.ExecutingContexts.None || $scope.CurrentExecutingContext == $scope.ExecutingContexts.Renaming)) {
                ...
                WebsiteManagerDataProvider.updateViewFieldsEnum($scope.currentObj).then(function (returnValue) {
                    UserMessagesProvider.hideLoading();
                    if (returnValue.data.result == 'true') {
    ...
    $scope.saveFields = function () {
        UserMessagesProvider.displayLoading();
        var distNode = listsTreeContainer.jstree(true).get_node($scope.currentNodeObject);
        WebsiteManagerDataProvider.updateViewFieldsEnumValues($scope.currentObj.Id, { Values: $scope.currentObj.Values }).then(function (returnValue) {
            UserMessagesProvider.hideLoading();
            if (returnValue.data.result == 'true') {
    ...
    $scope.removeField = function (index) {
        ...
        UserMessagesProvider.confirmHandler("Are you sure you want to delete this item?", function () {
            var distNode = listsTreeContainer.jstree(true).get_node($scope.currentNodeObject);
            UserMessagesProvider.displayLoading();
            WebsiteManagerDataProvider.deleteViewFieldsEnumValue($scope.currentObj.Values[index].Id).then(function (returnValue) {
                UserMessagesProvider.hideLoading();
                if (returnValue.data.result == 'true') {
                    $scope.currentObj.Values.splice(index, 1);
