appRoot.controllerProvider.register('Membership.RolesListController', function ($scope, $location, BreadCrumpsProvider, UserMessagesProvider, MembershipDataProvider, EventsProvider, $modal, filterFilter) {
    ...
    $scope.deleteRole = function (index) {
        ...
        UserMessagesProvider.confirmHandler("Are you sure you want to delete this role?", function () {
            UserMessagesProvider.displayLoading();
            MembershipDataProvider.deleteRole($scope.currentRole).success(function (returnValue) {
                UserMessagesProvider.hideLoading();
    ...
    $scope.createRole = function () {
        if ($scope.createForm.$valid) {
            UserMessagesProvider.displayLoading();
            MembershipDataProvider.createRole($scope.currentRole).success(function (data) {
                if (data.result == 'true') {
    ...
    $scope.editRole = function () {
        if ($scope.editForm.$valid) {
            UserMessagesProvider.displayLoading();
            MembershipDataProvider.updateRole($scope.currentRole).success(function (data) {
                if (data.result == 'true') {
