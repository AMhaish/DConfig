appRoot.controllerProvider.register('Membership.PagesPriviligesController', function ($scope, $location, BreadCrumpsProvider, UserMessagesProvider, MembershipDataProvider, EventsProvider, $modal, scopeService) {
    ...
    function initialize() {
        ...
        MembershipDataProvider.getContentsTree().success(function (data) {
            $scope.privilegesTree = data;
    ...
    $scope.updatePrivilege = function () {
        ...
        if ($scope.currentNodeObject.original.addObj == null) {
            ...
            MembershipDataProvider.createContentPrivilege($scope.currentPrv, contentId).success(function (returnValue) {
                UserMessagesProvider.hideLoading();
                if (returnValue.result == 'true') {
                    distNode.original.addObj = returnValue.obj;
                    $scope.currentPrv = jQuery.extend(true, {}, $scope.currentNodeObject.original.addObj);
                }
                else {
          ...
          .error(function (errorData, status, headers, config) {
            ...
            if (!$scope.currentPrv.NeedAuthentication && ($scope.currentPrv.Roles == undefined || $scope.currentPrv.Roles.length <= 0) && !$scope.currentPrv.RequireHttps) {
                MembershipDataProvider.deleteContentPrivilege($scope.currentPrv.ContentId).success(function (returnValue) {
                    UserMessagesProvider.hideLoading();
                    if (returnValue.result == 'true') {
              ...
              .error(function (errorData, status, headers, config) {
                ...
                MembershipDataProvider.updateContentPrivilege($scope.currentPrv).success(function (returnValue) {
                    UserMessagesProvider.hideLoading();
                    if (returnValue.result == 'true') {
                        distNode.original.addObj = returnValue.obj;
                        $scope.currentPrv = jQuery.extend(true, {}, $scope.currentNodeObject.original.addObj);
    ...
    $scope.addPrivilegeToRole = function () {
        ...
        modalInstance.result.then(function (obj) {
            ...
            if (!exists) {
                UserMessagesProvider.displayLoading();
                MembershipDataProvider.addContentPrivilegeToRole($scope.currentPrv.ContentId, obj.Name).success(function (returnValue) {
                    UserMessagesProvider.hideLoading();
                    if (returnValue.result == 'true') {
    ...
    $scope.removePrivilegeFromRole = function (index) {
        UserMessagesProvider.confirmHandler("Are you sure you want to remove the role from this privilege?", function () {
            var distNode = privilegesTreeContainer.jstree(true).get_node($scope.currentNodeObject);
            UserMessagesProvider.displayLoading();
            MembershipDataProvider.removeContentPrivilegeFromRole($scope.currentPrv.ContentId, $scope.currentPrv.Roles[index].Name).success(function (returnValue) {
                UserMessagesProvider.hideLoading();
                if (returnValue.result == 'true') {
