appRoot.controllerProvider.register('Membership.PriviligesController', function ($scope, $location, BreadCrumpsProvider, UserMessagesProvider, MembershipDataProvider, EventsProvider, $modal, scopeService) {
    ...
    function initialize() {
        ...
        MembershipDataProvider.getSystemControllersTree().success(function (data) {
    ...
    $scope.updatePrivilege = function () {
        ...
        if ($scope.currentNodeObject.original.addObj == null || $scope.sameAsParent) {
            ...
            MembershipDataProvider.createPrivilege($scope.currentPrv, controllerName, actionName, requestType).success(function (returnValue) {
                UserMessagesProvider.hideLoading();
            ...
            if (becomeSameAsParent && sameRoles) {
                MembershipDataProvider.deletePrivilege($scope.currentPrv).success(function (returnValue) {
                    UserMessagesProvider.hideLoading();
                    if (returnValue.result == 'true') {
              ...
              .error(function (errorData, status, headers, config) {
                ...
                MembershipDataProvider.updatePrivilege($scope.currentPrv).success(function (returnValue) {
                    UserMessagesProvider.hideLoading();
                    if (returnValue.result == 'true') {
                        distNode.original.addObj = returnValue.obj;
                        $scope.closeDetailsForm();
    ...
    $scope.addPrivilegeToRole = function () {
        ...
        modalInstance.result.then(function (obj) {
            ...
            if ($scope.currentNodeObject.original.type == "Container" || $scope.sameAsParent == false) {
                ...
                if (!exists) {
                    UserMessagesProvider.displayLoading();
                    MembershipDataProvider.addPrivilegeToRole($scope.currentPrv.Id, obj.Name).success(function (returnValue) {
                        UserMessagesProvider.hideLoading();
                        if (returnValue.result == 'true') {
    ...
    $scope.removePrivilegeFromRole = function (index) {
        UserMessagesProvider.confirmHandler("Are you sure you want to remove the user from this role?", function () {
            ...
            if ($scope.currentNodeObject.original.type == "Container" || $scope.sameAsParent==false) {
                UserMessagesProvider.displayLoading();
                MembershipDataProvider.removePrivilegeFromRole($scope.currentPrv.Id, $scope.currentPrv.Roles[index].Name).success(function (returnValue) {
                    UserMessagesProvider.hideLoading();
                    if (returnValue.result == 'true') {
